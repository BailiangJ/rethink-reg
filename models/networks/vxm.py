        ...
        if in_channels > 0:
            if up_transp_conv:
                ...
                if spatial_dims == 2:
                    ...
                    raise KeyError(
                        f"Unsupported spatial dimension for Upsample:{spatial_dims}."
                    )
                self.upsample = nn.Upsample(
                    scale_factor=upsample_kernel_size, mode=mode, align_corners=True
                )
...
@FLOW_ESTIMATORS.register_module()
class VXMPlus_Dual(VXMPlus):
    def __init__(
        self, encoder_cfg, decoder_cfg, remain_cfg, bidirectional: bool = False
    ):
    ...
    def forward(self, src: torch.Tensor, tgt: torch.Tensor) -> torch.Tensor:
        ...
        feats = [
            torch.cat([src_feat, tgt_feat], dim=1)
        ...
        if self.bidirectional:
            bck_feats = [
                torch.cat([tgt_feat, src_feat], dim=1)
                for src_feat, tgt_feat in zip(src_feats, tgt_feats)
            ]
