...
def window_partition(x, window_size):
    ...
    # permute: [B, H//Mh, Mh, W//Mw, Mw, C] -> [B, H//M, W//M, M, M, C]
    # view: [B, H//Mh, W//Mw, Mh, Mw, C] -> [num_windows*B, Mh, Mw, C]
    windows = (
...
def window_reverse(windows, window_size, H, W):
    ...
    # view: [B, H//Mh, W//Mw, Mh, Mw, C] -> [B, H//Mh, Mh, W//Mw, Mw, C]
    x = windows.view(
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        """
        B_, N, C = x.shape  # (num_windows*B, Wh*Ww, C)
        # qkv(): -> (num_windows*B, Wh*Ww, C*3)
        # reshape: -> (num_windows*B, Wh*Ww, 3, num_heads, C//num_heads)
        # permute: -> (3, num_windows*B, num_heads, Wh*Ww, C//num_heads)
        qkv = (
            self.qkv(x)
        ...
        # @ - multiply: -> (num_windows*B, num_heads, Wh*Ww, Wh*Ww)

        q = q * self.scale
        attn = q @ k.transpose(-2, -1)

        if self.rpe:
            # view: [Wh*Ww*Wh*Ww, nH] -> [Wh*Ww, Wh*Ww, nH]
        ...
        if mask is not None:
            ...
            nW = mask.shape[0]
            # view: [num_windows*B, num_heads, Wh*Ww, Wh*Ww] -> [B, nW, num_heads, Wh*Ww, Wh*Ww]
            ...
            )  # view: -> (num_windows*B, num_heads, Wh*Ww, Wh*Ww)
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # pad from the last dimension and move backwards
        # (W_front, W_back, H_front, H_back)
        if pad_input:
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            ...
            # flatten: B C Wh Ww -> B Wh*Ww C
            # transpose: B Wh*Ww C -> B Wh*Ww C
...
class TransMorph2D(nn.Module):
    def __init__(self, config):
        ...
        TransMorph Model
        """
        super(TransMorph2D, self).__init__()
    ...
    def forward(self, source: torch.Tensor, target: torch.Tensor):
        """ "
        Args:
            source (torch.tensor): [BCHW]
            target (torch.tensor): [BCHW]
        Return:
            flow (torch.tensor): [B, 3, Hf, Wf]
        """
