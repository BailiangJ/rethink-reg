...
class BasicEncoder(BaseModule):
    ...
    def __init__(self,
        ...
        assert len(out_channels) == len(num_convs) == len(strides) == len(
            dilations) == len(pyramid_levels), f"the length of out_channels, num_convs, strides, dilations and pyramid_levels must be the same, but got {len(out_channels)}, {len(num_convs)}, {len(strides)}, {len(dilations)} and {len(pyramid_levels)}"
