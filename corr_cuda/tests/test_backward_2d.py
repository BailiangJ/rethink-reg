...
def test_backward_2d_float32(padding, scale):
    torch.manual_seed(19)
    x = torch.randn(1, 3, 5, 6, device="cuda", requires_grad=True)
    y = torch.randn(1, 3, 5, 6, device="cuda", requires_grad=True)
    _compare_grads(x, y, radius=2, ndim=2, padding=padding, scale=scale)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_backward_2d_norm_vectors():
    torch.manual_seed(23)
    x = torch.randn(1, 4, 5, 6, device="cuda", requires_grad=True)
    y = torch.randn(1, 4, 5, 6, device="cuda", requires_grad=True)
    _compare_grads(x, y, radius=1, ndim=2, padding="constant", scale=True, norm_vectors=True)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
@pytest.mark.parametrize("x_requires_grad,y_requires_grad", [(True, False), (False, True)])
def test_backward_2d_skips_unused_input_grad(x_requires_grad, y_requires_grad):
    torch.manual_seed(67)
    x = torch.randn(1, 3, 5, 6, device="cuda", requires_grad=x_requires_grad)
    y = torch.randn(1, 3, 5, 6, device="cuda", requires_grad=y_requires_grad)
    xr = x.detach().clone().requires_grad_(x_requires_grad)
    yr = y.detach().clone().requires_grad_(y_requires_grad)

    out_cuda = WinCorrCuda(radius=2, ndim=2, padding="replicate", scale=True)(x, y)
    out_ref = ref_win_corr(xr, yr, radius=2, ndim=2, padding="replicate", scale=True)
